"""Checkpoint-free GDN-2 recurrent forward (inference fast-path, A2 / c220).

Same recurrence and arithmetic as ``fwd_states.py`` but without the per-step
``states`` / ``delta_ckpt`` writes the backward needs — so it outputs only
``o`` and ``final_state`` and allocates no 68 MB checkpoint buffer. Used by the
no-grad branch of ``gdn2_recurrent``: the outputs are bit-identical to the
training forward (``torch.equal``), and dropping the checkpoint allocation +
autograd node cuts ~120 µs of host-side per-call overhead, so the a2 eager wall
beats fla's Triton ``fused_recurrent`` (301 vs 328 µs at 1×64×16). See
BENCHMARK_TRITON.md.
