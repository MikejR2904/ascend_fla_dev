...
export PYTHONPATH=/workspace/ascriptor-ws/library:/workspace/ascend_fla_dev
export ASCRIPTOR_WORKSPACE=/workspace/ascriptor-ws
export ASCEND_RT_VISIBLE_DEVICES=7
MSPROF=/usr/local/Ascend/cann-9.2.0-beta.1/bin/msprof
ROOT=/tmp/gdn2_prof
...
for KERN in fwd bwd; do for SHP in decode train batch8; do
  ...
  timeout 900 $MSPROF --application="python3 /tmp/bench_msprof.py --kernel $KERN --shape $SHP --iters $ITERS --warmup 10" \
