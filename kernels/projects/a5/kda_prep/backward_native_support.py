...
def ieee_four_columns(candidate, old_npu, cpu_fp32, fp64):
    """Pointwise observations; equality labels do not establish accuracy passes."""
    tensors = {n: t.detach().cpu().contiguous() for n, t in
               [('candidate', candidate), ('old_npu', old_npu), ('cpu_fp32', cpu_fp32), ('fp64', fp64)]}
    assert tensors['cpu_fp32'].dtype == torch.float32
    assert tensors['fp64'].dtype == torch.float64
    assert len({tuple(t.shape) for t in tensors.values()}) == 1
    rows = []
    for flat in range(candidate.numel()):
        columns = {}
        for name, tensor in tensors.items():
            value = tensor.reshape(-1)[flat]
            width = value.element_size() * 8
            integer = {16: torch.int16, 32: torch.int32, 64: torch.int64}[width]
            bits = int(value.reshape(1).view(integer)[0]) & ((1 << width)-1)
            classification = ('nan' if value.isnan() else 'negative_inf' if value.isneginf() else
                'positive_inf' if value.isposinf() else 'zero' if value == 0 else
                'subnormal' if value.abs() < torch.finfo(value.dtype).tiny else 'normal')
            columns[name] = dict(dtype=str(value.dtype), bits=f'{bits:0{width//4}x}',
                value=float(value) if value.isfinite() else None, ieee_class=classification)
        def equal(a, b):
            a, b = tensors[a].reshape(-1)[flat], tensors[b].reshape(-1)[flat]
            return bool((a.double() == b.double()) | (a.isnan() & b.isnan()))
        cpu_equal, npu_equal = equal('candidate', 'cpu_fp32'), equal('candidate', 'old_npu')
        index, rest = [], flat
        for extent in reversed(candidate.shape):
            index.insert(0, rest % extent); rest //= extent
        rows.append(dict(index=index, **columns,
            candidate_equals_cpu=cpu_equal, candidate_equals_old_npu=npu_equal,
            candidate_equals_old_npu_differs_cpu=npu_equal and not cpu_equal,
            old_npu_differs_candidate_equals_cpu=cpu_equal and not npu_equal,
            candidate_only_nan=columns['candidate']['ieee_class'] == 'nan'
                and columns['cpu_fp32']['ieee_class'] != 'nan' and columns['old_npu']['ieee_class'] != 'nan'))
    return rows
...
def gradient_record(key, actual, fp64, old, precision, budget, detail_root,
                    ...
                    native_flush_mask=None, cpu_fp32=None):
    ...
    old_ref_ulps, old_ref_finite, old_ref_histogram = numeric_ulps(old, fp64)
    ordinary = torch.ones_like(actual, dtype=torch.bool)
    flush = None
    if native_flush_mask is not None:
        mask = native_flush_mask.detach().cpu()
        assert mask.dtype == torch.bool and mask.shape == actual.shape
        assert cpu_fp32 is not None and cpu_fp32.dtype == torch.float32
        golden = cpu_fp32.detach().cpu()
        assert golden.shape == actual.shape
        valid = (golden.abs() > 0) & (golden.abs() < torch.finfo(torch.float32).tiny) & (actual == 0)
        flush = dict(authority='D-PM-56(2), D-PM-50', elements=int(mask.sum()),
            classification_valid=bool(valid[mask].all()), cpu_correctness_pass=False,
            status='native_underflow_disclosure; not CPU correctness pass')
        ordinary &= ~mask
    endpoint = None
    ordinary_high = high
    ...
    if endpoint_zero_mask is not None:
        ...
        if native_flush_mask is not None:
            assert not bool((mask & native_flush_mask.cpu()).any())
        ordinary &= ~mask
    ...
    if endpoint_zero_mask is not None or native_flush_mask is not None:
    ...
    old_line_required = ordinary & old_ref_finite & (old_ref_ulps <= 1)
    old_line_disclosed = ordinary & old_ref_finite & (old_ref_ulps > 1)
    if budget['ulp_limit_each_reference'] is not None:
        # D-PM-56(1): the FP64 line always applies. The predecessor line
        # applies only where that predecessor itself lies within one ULP of
        # FP64; other locations remain explicit observations, not wider limits.
        passed = passed and bool(finite64[ordinary].all() and old_ref_finite[ordinary].all())
        passed = passed and not bool((u64[ordinary] > 1).any())
        passed = passed and bool(finite_old[old_line_required].all()) and not bool((uold[old_line_required] > 1).any())
    ...
    classified = passed and (flush is None or flush['classification_valid'])
    if flush is not None and flush['elements']:
        passed = False  # A correctly classified native flush is still not a CPU pass.
    details = []
    if actual.dtype == torch.bfloat16:
        report_mask = (finite64 & (u64 > 1)) | (finite_old & (uold > 1))
        if budget['ulp_limit_each_reference'] is not None:
            report_mask = report_mask | old_line_disclosed
        locations = report_mask.nonzero().tolist()
        ...
        for index in locations:
            ...
            row = dict(index=index, candidate_bits=int(actual.view(torch.int16)[ix]) & 65535,
                       ...
                       old_to_fp64_ulp=int(old_ref_ulps[ix]),
                       old_line_disclosure=bool(old_line_disclosed[ix]) if budget['ulp_limit_each_reference'] is not None else False)
    ...
    return dict(key=key, to_fp64=high, to_old_cpu=previous, budget=budget,
                ...
                native_flush=flush, classification_complete=bool(classified),
                ulp_to_fp64_distribution=histogram64, ulp_to_old_distribution=histogram_old,
                old_to_fp64_ulp_distribution=old_ref_histogram,
                old_host_ulp_policy=dict(authority='D-PM-56(1); issue117 specification',
                    required_elements=int(old_line_required.sum()),disclosed_elements=int(old_line_disclosed.sum()))
                    if budget['ulp_limit_each_reference'] is not None else None,
