...
def main():
    ...
    def compare(name,key,got,high,before,condition=None):
    ...
    try:
        if args.kind=='norm':
            ...
            compare('norm65rows','norm:'+args.dtype+':dx',got,high,previous,cond)
            ...
            got=launch('beta_'+args.dtype,(probability.reshape(1,-1),gy.reshape(1,-1),out,x.numel())).reshape(x.shape)
            ...
            for bt,hv in ((65,2),(1,17)):
                ...
                n=x.numel();chunks=(bt+31)//32;p=hv*chunks*256
