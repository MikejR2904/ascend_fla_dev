"""CPU-only BF16 input generation and independent FP32 references."""
...
def validate_inputs(inputs, case=None):
    ...
    for name, shape in shapes.items():
        x = inputs[name]
        if not isinstance(x, torch.Tensor) or tuple(x.shape) != shape or x.dtype != (torch.bfloat16 if name in ('q','k','v') else torch.float32):
    ...
    if bool((inputs['k'].float().square().sum(-1) > (1.0 + 1e-5)**2).any()):
...
def reference(inputs):
    ...
    stages = reference_stages(fp32_inputs(inputs))
...
def fp32_inputs(inputs):
    """Reference/test generation only. Never called by the public operator."""
    return {n:(x.float() if isinstance(x,torch.Tensor) else x) for n,x in inputs.items()}
