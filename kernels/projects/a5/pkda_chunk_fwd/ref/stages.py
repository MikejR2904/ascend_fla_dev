...
def reference_stages(inputs):
    ...
    chunks = (time + 63) // 64
    qn = q * float(inputs.get('scale', 128**-0.5))
    k = inputs['k']  # Pinned naive semantics: inputs are not normalized here.
    a = inputs['initial_A_state'].clone()
    center = inputs['log_atk_scale'].view(1, heads, 1)
    preconditioned = []
    logx = torch.log(torch.tensor(1.5, dtype=torch.float32))
    for t in range(time):
        a = inputs['g_atk'][:, t].exp().unsqueeze(-1) * a + inputs['beta_atk'][:, t].unsqueeze(-1) * k[:, t].square()
        r = (a + 1e-6).log() - center
        m = (-logx * (r / (1.0 + r.abs()))).exp()
        preconditioned.append(k[:, t] * m)
    kn = torch.stack(preconditioned, dim=1)
    def pack(x):
    ...
    qn, kn, gc, bk, wv = (
        ...
        pack(inputs['beta'].unsqueeze(-1) * k), pack(inputs['beta'].unsqueeze(-1) * inputs['v']))
    ...
    return dict(qn=qn, kn=kn, gc=gc, bk=bk, wv=wv, lower=lower, score=score,
                ...
                delta=torch.stack(deltas, dim=1), final_state=state, final_A_state=a, o=o)
