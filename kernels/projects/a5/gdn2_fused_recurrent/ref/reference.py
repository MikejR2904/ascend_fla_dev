...
def validate_inputs(inputs: dict[str, torch.Tensor], case: dict[str, Any] | None = None) -> None:
    ...
    if batch != 1 or not 1 <= time <= 16 or heads not in (1, 16) or width != HEAD_DIM:
    ...
    if case is not None and int(case.get("block_dim", 8)) != 8:
        raise ValueError("this unit fixes block_dim=8")
