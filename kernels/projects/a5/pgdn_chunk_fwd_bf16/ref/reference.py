...
def cases():
    """One seed/shape per pair of launch configurations."""
    contract = json.loads((Path(__file__).parents[1] / 'contract.json').read_text())
    return [case for case in contract['cases'] if case['block_dim'] == 1]


def metric(actual, expected):
    if actual.shape != expected.shape:
        raise ValueError('comparison requires matching shapes')
    a, e = actual.double(), expected.double()
    error, norm = (a - e).norm().item(), e.norm().item()
    return dict(relative_l2=error / norm if norm else (0.0 if error == 0 else float('inf')),
...
def budget(expected, dtype, name):
    if dtype == torch.bfloat16 and name in ('o', 'final_state'):
        return min(1e-2, 3 * floor(expected))
    return 1e-4


def acceptable(actual, expected, dtype):
    if set(actual) != set(expected) or set(actual) != set(OUTPUTS):
        return False
    for name in OUTPUTS:
        required_dtype = dtype if name == 'o' else torch.float32
        if actual[name].shape != expected[name].shape or actual[name].dtype != required_dtype:
            return False
        result = metric(actual[name], expected[name])
        if not result['finite'] or result['relative_l2'] > budget(expected[name], dtype, name):
            return False
    return True
...
def make_inputs(case):
    ...
    if dtype not in (torch.bfloat16, torch.float32):
        raise ValueError('reference generator requires bfloat16 or float32')
    for name in ('q', 'k', 'v'):
...
def validate_inputs(inputs, case=None):
    ...
    if q.dtype not in (torch.bfloat16, torch.float32):
        raise ValueError('q/k/v require matching bfloat16 or float32')
    for name, value in inputs.items():
        dtype = q.dtype if name in ('q', 'k', 'v') else torch.float32
        if value.shape != shapes[name] or value.dtype != dtype or value.device.type != 'cpu' or not value.is_contiguous():
            raise ValueError(f'{name} requires contiguous CPU {dtype} {tuple(shapes[name])}; got {tuple(value.shape)}/{value.dtype}')
