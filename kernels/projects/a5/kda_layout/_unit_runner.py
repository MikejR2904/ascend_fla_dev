...
def comparison_rule(contract: dict, name: str, stage: bool = False) -> dict:
    ...
    if rule.get("mode") not in {"bitwise", "exact", "allclose"}:
        raise ContractError(f"comparison for {name} requires mode bitwise, exact or allclose")
...
def compare_outputs(actual: dict, expected: dict, contract: dict, stage: bool = False) -> dict:
    import numpy as np
...
def run(root: Path, argv: list[str] | None = None) -> dict:
    ...
    if all(row["status"] == "gap" for row in rows):
        raise ContractError("located support gap: " + "; ".join(row["reason"] for row in rows))
    ...
    try:
        for declared_case in cases:
            ...
            if parsed.command != "reference":
                ...
                entry["comparison"] = compare_outputs(actual, expected, contract)
