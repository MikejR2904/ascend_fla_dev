...
__aicore__ inline void a201_splitk_bf16_m16_n64_k128_s16_cube(GM_ADDR x_, GM_ADDR y_, GM_ADDR z_, GM_ADDR workspace)
{
    GMTensor<bfloat16_t> x((__gm__ bfloat16_t*)x_);
    GMTensor<bfloat16_t> y((__gm__ bfloat16_t*)y_);
    GMTensor<float> z((__gm__ float*)z_);
    SEvent<PIPE_MTE2, PIPE_MTE1, 0, 0> ev_mte2_mte1_ready_1;  // guards l1x, l1y #49
    SEvent<PIPE_M, PIPE_FIX, 0, 0> ev_m_fix_ready_1;  // guards l0c #50
    DEvent<PIPE_M, PIPE_MTE1, 2, 0, 1> l0_splitk_available;  // guards _l0a, _l0b #39
    DEvent<PIPE_MTE1, PIPE_M, 0, 0, 1> l0_splitk_ready;  // guards _l0a, _l0b #40
    ...
    const Tensor<bfloat16_t, Position::L1> l1y((uint64_t)(4096));  // #2
    ...
    for (int32_t _subk = 0; _subk < 128; _subk += 16) {  // #38
