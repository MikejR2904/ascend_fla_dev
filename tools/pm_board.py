...
TRACK_LABEL = {
    "zh": {"agent": "可申领", "owner": "仓主轨道", "upstream": "上游单元", "none": "尚未排任务"},
    "en": {"agent": "open to agents", "owner": "owner track", "upstream": "upstream unit", "none": "no task yet"},
}
_T = {
    "zh": {"gen": "由 tools/pm_board.py --readme 生成，请勿手改。改 docs/pm/board.json 后重新运行。",
           "head": "| kernel | 算子族 | 归属 | 进度 | 下一步 |", "model": "目标模型",
           "cols": "| 任务 | issue | SoC | dtype | 状态 | 说明 |",
           "done": "完成", "start": "起点", "nostart": "无起点", "startflag": "★ 起点",
           "detail": "各 kernel 的任务链",
           "legend": ["> 图例：⬜ 可申领 · 🔵 进行中 · 🔒 有前置条件未满足 · ✅ 已完成 · — 该 kernel 还没有任务。",
...
def render_readme_block(board: dict, lang: str = "zh") -> str:
    ...
    for k in inventory:
        kernel, starts = k["id"], entries.get(k["id"], [])
        chain = chain_of(board, kernel)
        track = TRACK_LABEL[lang][k["track"]]
        if chain:
            done = sum(t["status"] == "done" for t in chain)
            nxt = next((t for t in chain if t["id"] == starts[0]), None) if starts else None
            progress, nxt_cell = f"{done}/{len(chain)}", (f"{link(nxt)} {nxt['id']}" if nxt else "—")
        else:
            progress, nxt_cell = "—", f"_{k['note_zh' if lang == 'zh' else 'note_en']}_"
        lines.append(f"| `{kernel}` | {k['family']} | {track} | {progress} | {nxt_cell} |")
    ...
    for k in inventory:
        ...
        lines += [f"<details><summary><b>{kernel}</b> —— {done}/{len(chain)} {tr['done']}，{start_txt}</summary>",
                  "", tr["cols"], "|---|---|---|---|---|---|"]
        for t in chain:
