...
def check(board: dict, root: Path = ROOT) -> list[str]:
    ...
    problems += _check_dtype_status(board)
...
# 每个 kernel 的 BF16 / FP32 现状（用户 2026-09-19 指出：进展表没有按 kernel 列 dtype）。
# 取值：native[@a2|a3|a5|host] = kernel 以该 dtype 计算（@ 后是验证在哪）；widen[@…] = 公共入口收该 dtype、host 侧加宽成 FP32；
# reject = 公共入口显式拒绝；wip = 进行中；none = 未实现；unknown = PM 没核（仓主轨道 / 上游单元的细节）。
DTYPE_STATE = re.compile(r"^(?:(?:native|widen)(?:@(?:a2|a3|a5|host))?|reject|wip|none|unknown)$")
_DTYPE_WORD = {
    "zh": {"native": "✅ 原生", "widen": "🔁 API 加宽", "reject": "⛔ 拒绝", "wip": "🔵 进行中", "none": "—", "unknown": "❓ 未核",
           "a5": "A5 真机", "a2": "A2 真机", "a3": "A3 真机", "host": "仅主机侧", "": "未上机"},
    "en": {"native": "✅ native", "widen": "🔁 API-widen", "reject": "⛔ rejected", "wip": "🔵 in progress", "none": "—", "unknown": "❓ unchecked",
           "a5": "A5 hw", "a2": "A2 hw", "a3": "A3 hw", "host": "host only", "": "not on hw"},
}


def dtype_cell(value: str | None, lang: str) -> str:
    if not value:
        return _DTYPE_WORD[lang]["unknown"]
    base, _, where = value.partition("@")
    word = _DTYPE_WORD[lang][base]
    if base in ("native", "widen"):
        word += f" · {_DTYPE_WORD[lang][where]}"
    return word


def _check_dtype_status(board: dict) -> list[str]:
    problems = []
    task_ids = {t["id"] for t in board.get("tasks", [])}
    for k in (board.get("kernel_inventory") or {}).get("kernels", []):
        for d, tid in (k.get("dtype_fix") or {}).items():
            if d not in ("bf16", "fp32") or tid not in task_ids:
                problems.append(f"kernel_inventory/{k['id']}: dtype_fix.{d} 必须指向看板上存在的任务，实际 {tid!r}")
        ds = k.get("dtype_status")
        if ds is None:
            continue
        for d, v in ds.items():
            if d not in ("bf16", "fp32"):
                problems.append(f"kernel_inventory/{k['id']}: dtype_status 只认 bf16 / fp32，实际 {d!r}")
            elif not isinstance(v, str) or not DTYPE_STATE.match(v):
                problems.append(f"kernel_inventory/{k['id']}: dtype_status.{d} 取值非法 {v!r}")
    return problems
...
_T = {
    "zh": {"gen": "由 tools/pm_board.py --readme 生成，请勿手改。改 docs/pm/board.json 后重新运行。",
           ...
           "kcols": "| kernel | 归属 | BF16 | FP32 | 进度 | 下一步 |",
           ...
           "legend": ["> 图例：⬜ 可申领 · 🔵 进行中 · 🔒 有前置条件未满足 · ✅ 已完成 · — 还没有任务。",
                      ...
                      "> 🔁 API 加宽 = 公共入口收该 dtype，但在 host 侧加宽成 FP32 再进 FP32 kernel、输出转回，**kernel 本身不跑该 dtype**；"
                      "**不合规（用户 D-PM-35：BF16 必须 kernel 侧），`→ BF-xx` 是改正它的任务**；",
    ...
    "en": {"gen": "Generated by tools/pm_board.py --readme. Do not edit by hand; edit docs/pm/board.json and re-run.",
           ...
           "kcols": "| kernel | track | BF16 | FP32 | progress | next |",
           ...
           "legend": ["> Legend: ⬜ open · 🔵 in progress · 🔒 blocked by a gate · ✅ done · — no task yet.",
                      ...
                      "> 🔁 API-widen = the public entry accepts that dtype but widens it to FP32 on the host, runs FP32 kernels and casts the output back — **the kernel itself does not run that dtype**; "
                      "**non-compliant since D-PM-35 (BF16 must be computed in kernels); `→ BF-xx` is the task that fixes it**;",
    ...
    for f in families:
        ...
        if ks:
            ...
            for k in ks:
                ...
                ds, fix = k.get("dtype_status") or {}, k.get("dtype_fix") or {}
                cells = [dtype_cell(ds.get(d), lang) + (f" → {fix[d]}" if d in fix else "") for d in ("bf16", "fp32")]
                lines.append(f"| `{k['id']}` | {TRACK_LABEL[lang][k['track']]} | {cells[0]} "
                             f"| {cells[1]} | {progress} | {nxt_cell} |")
            lines.append("")
            for k in ks:
