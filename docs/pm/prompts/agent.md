...
- **BF16 计算必须在 kernel 里（用户 D-PM-35）**：BF16 张量直接进自编译 kernel，转换与算术在 kernel 里做，host 侧只允许零算术、零 dtype 转换的布局操作——**不得** `q.float()` 加宽再转回；
  做不到的路径显式拒绝。通则、host 算子审计与预算口径见 `docs/pm/bf16-kernel-side.md`。「后续 BF16 优先」：BF 系列任务是 P0。
