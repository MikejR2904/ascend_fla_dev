...
> **2026-09-21T02:45Z 更新：BF-08 首次真机（候选实现，PR #122 头 ccc7194，仍 draft）——norm 逐元素相对误差超冻结限（q 0.0774、k 2.166，限值 0.0588），PM 核实是判据度量的问题，不是候选缺陷；待用户：逐元素判据怎么改（数据到齐后发决策包）。**
...
> - PM 已回复申领人：别为它改 kernel 算术、别动冻结预算文件；补同人口成对数据 + 按项幅值归一数据 + 校准人口上旧 host 的稳定性。**我不在实现之后自己改冻结判据**；数据到齐后向用户提两个方向之一（成对比较 / 项幅值归一）。
