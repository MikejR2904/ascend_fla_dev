...
Current qualification: CPU precision research passed for the selected candidate. Source emission, vendor compilation, simulator, pipe simulator, native correctness, host audit and performance are all UNTESTED. Historical PK-04 results confer no BF-04 qualification.

## D-PM-37: both dtype paths audited

The original FP32 path has no layout conversion, but its numerical validator invokes Torch arithmetic. The before-validator CPU trace includes abs/ne/eq/mul/all/gt/lt/bitwise_or/any/pow/sum/neg and scalar extraction; this is a located conflict with the new all-dtype audit, reported to PM. The new unit adds one FP32 device guard/default-initializer launch before the unchanged five FP32 mathematical kernels. It copies states without transformation, creates exact zero/default-center values, and checks the same numeric domain. Native before/after byte equality and both host audits remain pending. Twelve vendors (six BF16, five unchanged FP32, one new FP32 guard) must compile before first custom execution.
...
Initial source emission succeeds for all six BF16 kernels and the new FP32 guard. Output ND-to-NZ transfers carry a performance advisory: four BF16 operand publications use multiple MTE3 bursts, with bounded legal footprints and explicit ownership. The first candidate keeps this sanctioned transfer to obtain full native correctness evidence before any layout optimization; no speed claim is made. The emitted output cube uses local mutex IDs0..9, below the fixed32 limit, and explicit logical cross-side IDs0..4.
