...
`stable` 下前向 155、反向 100，纯推理走前者、训练走后者。默认初始化的 94 两条都满足。
...
跨度 94（默认初始化）六项全在预算内，110 过但 `dq` 只剩 0.2% 余量，130 起超。
闸因此取 100。
...
4. **"不吐 NaN"不等于"能用"。** 反向的有限性上限是 169.8，而精度在 130 就超出契约预算 ——
