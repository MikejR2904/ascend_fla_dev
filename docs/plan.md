...
用户决定 SoC 顺序改为 **A2 (910B) → A3 (910C) → A5**，首波模型 Kimi-Linear（KDA）然后 Qwen3-Next（GDN），
工具链只用 ascriptor。第三、四期的内容不删，按下表重新归入波次；逐项任务、依赖与验收在 `docs/pm/board.json`。
...
**最大的风险**：ascriptor 侧 A2/A3 是 deferred 状态，且带未解决的 split-K FP32 cube 数值缺陷。W-A2 的所有算子任务
都依赖 A2-01（主机侧定性）与 A2-11（真机结论）；若没有可行绕行，是否退回 A5 由用户决定。
