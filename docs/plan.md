...
输入域（`g_raw ∈ [-0.03, 0]`）只到 1.92 —— 窄 50 倍。ascriptor 的 kernel 在两处撑不住，
**方向相反**：
...
改取**中点**，两个因子各压到 `exp(±span/2)`，上限正好翻倍：前向 `2 × 87.3 ≈ 174.7`，
反向 `2 × 88.72 ≈ 177.4`，`MAX_GATE_SPAN["stable"]` 取 160 作公共安全值。
...
**还差一项**：默认初始化（跨度 ~94）下整层反向的精度。它需要有 `ascend950` 算子包的机器
（层里的投影/卷积/softplus 都是 torch_npu 算子），测试已写好 ——
`tests/test_kda_layer_npu.py::test_default_init_backward_matches_cpu_reference`。
算子层面的证据已齐，所以这是确认而不是未知。

**三条教训。**

1. **窄域全绿不代表算子可用。** 前向五个 case、反向五个 case 全部 passed，跨度却只到 1.92。
   真实初始化在 94。测试覆盖的是契约声明的域，而契约声明的域可能根本不是模型会用的域 ——
...
2. **第一次归因错了，而且错在方向上。** 我最初把前向的失效写成 `ln(FLT_MAX)` 的**上溢**，
   ...
   `gaps.json` 里没有删。
3. **修一处不等于修完**。前向根治之后我差点就收工了 —— 反向那处是读源码时顺手发现的，
   不是测出来的，而契约的五个 case 永远测不到它（跨度 ≤1.92，离失效线 46 倍）。
